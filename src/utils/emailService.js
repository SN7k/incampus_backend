...
const generateOTP = (email) => {
  ...
  // In production, always generate a random OTP
...
const verifyOTP = (email, otp) => {
  ...
  // In development mode, accept the fixed OTP regardless of what's in the store
  if (process.env.NODE_ENV === 'development' && otp === DEV_OTP) {
    console.log('Development mode: accepting fixed OTP');
...
const sendOTPEmail = async (email, name) => {
  ...
  try {
    ...
    // For development/testing, bypass actual email sending and just log the OTP
    if (process.env.NODE_ENV === 'development') {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`DEVELOPMENT MODE: Despite email error, OTP for ${email} is ${otp}`);
      return { success: true, message: 'OTP sent successfully (development mode)' };
    }
